...
def motortune(detectors, opa, use_tune_points, motors, spectrometer=None, *, md=None):
    ...
    scanned_motors = [m for m, params in motors.items() if params.get("method") == "scan"]
    ...
    for mot, params in motors.items():
        if params["method"] == "static":
            ...
            cyc *= cycler(getattr(opa, mot), np.linspace(params["center"] - params["width"]/2, params["center"] + params["width"]/2, params["npts"]))
    if spectrometer and spectrometer["device"]:
        if spectrometer["method"] == "static":
            ...
            cyc *= cycler(spectrometer["device"], np.linspace(spectrometer["center"] - spectrometer["width"]/2, spectrometer["center"] + spectrometer["width"]/2, spectrometer["npts"]))
